...
本项目完全离线运行，无法收集任何用户信息或获取用户输入数据。因此，本项目的贡献者并不知晓一切的用户输入和用户使用的模型，因此不对任何用户输入和模型负责。

本项目只是一个框架项目，本身不具备语音合成的功能，所有功能都只能通过用户自己训练的模型来实现。同时，本项目不附带任何模型，任何二次分发都与本项目的贡献者无关
