...
def process_one(filename, hmodel, f0p, device, autocc, diff=False, mel_extractor=None):
    ...
    if not os.path.exists(f0_path):
        f0_predictor = utils.get_f0_predictor(f0p, sampling_rate=sampling_rate, hop_length=hop_length, device=None,
                                              threshold=0.05)
...
def process_batch(file_chunk, f0p, autocc, diff=False, mel_extractor=None, device="cpu"):
...
def parallel_process(filenames, num_processes, f0p, diff, mel_extractor, device, autocc):
    with ProcessPoolExecutor(max_workers=num_processes) as executor:
        ...
        for i in range(num_processes):
            ...
            tasks.append(executor.submit(process_batch, file_chunk, f0p, autocc, diff, mel_extractor, device=device))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--f0_predictor', type=str, default="dio",
    ...
    logger.info("Using Automatic cache clearing: " + str(autocc))
    ...
    parallel_process(filenames, num_processes, f0p, args.use_diff, mel_extractor, device, autocc)
