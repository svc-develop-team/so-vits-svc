...
def main():
    ...
    parser.add_argument('-l', '--wav_scp', type=str, required=True, help="The wav scp of the input files")
    parser.add_argument('-t', '--trans', type=int, nargs='+', default=[0], help='音高调整，支持正负（半音）')
    parser.add_argument('-o', '--output_dir', type=str, required=True, help="path to the output dir")
    ...
    raw_dir, result_dir = os.path.join(args.output_dir, "raw"), os.path.join(args.output_dir, "results")
    infer_tool.mkdir([raw_dir, result_dir])
    ...
    with open(args.wav_scp) as f:
        ...
        uttids = [item[0] for item in input_files]
        clean_names = [item[1] for item in input_files]

    infer_tool.fill_a_to_b(trans, clean_names)
    for uttid, clean_name, tran in zip(uttids, clean_names, trans):
        raw_audio_path = os.path.join(args.output_dir, "raw", uttid + ".wav")
        infer_tool.format_wav(raw_audio_path)
        assert Path(clean_name).suffix == '.wav'
        os.system("cp {} {}".format(clean_name, raw_audio_path))
        ...
        for spk in spk_list:
            ...
            if type(spk) != type('aa'):
                spk = "spk_mix"
            res_path = os.path.join(args.output_dir, "results", f"{uttid}_{key}_{spk}{cluster_name}_{isdiffusion}.{wav_format}")
